...
- **What's stored:** for each completed Hold period — the recipient names entered when that Hold was created (frozen at that moment, not linked to any later edits), the start and end timestamps, and, per Circle/contact, which channel their message actually went out through (e.g. text message vs. the share sheet, and which app on iOS when the OS reports one) — process metadata about how a send happened, never the message text itself.
...
- **How long:** 90 days from creation — a standalone figure, not tied to any trial or subscription mechanism: long enough to cover a real illness cycle end-to-end, short enough to avoid indefinite retention of personal details. A note that's surfaced and "Used" is kept but not suggested again; "Don't remember" deletes it immediately rather than just marking it resolved. Turning Layer 1 off deletes every stored note immediately, regardless of age.
